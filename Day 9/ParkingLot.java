        ...
        for (int i = 0; i < floorsCount; i++) {
            ...
            for (Vehicle v : slots) {
                if (v == null) {
                    ...
                    String parkedAt = formatter.format(Instant.ofEpochMilli(v.getParkedTime()));
                    System.out.print("[" + v.getType() + " - " + v.getRegistrationNumber() + " - " + v.getModel() + " - " + v.getDriverName() + " - Parked at: " + parkedAt + "] ");
                }
            }
            System.out.println();
        }
